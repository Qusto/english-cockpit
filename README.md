...
2. Откройте его двойным кликом **или** (рекомендуется, для авто-сохранения в папку) поднимите локальный сервер:
   ...
   # затем откройте http://localhost:8787/cockpit.html
   ```
   ...
   > Порт **8787** (не 8765): 8765 зарезервирован под AnkiConnect в Cockpit 2.0.
...
## Cockpit 2.0 (в разработке)

- `extension/` — Chrome-расширение: `Option + hover` на слове → перевод/контекст, клавиша `A` → карточка в Anki через AnkiConnect (`localhost:8765`). Установка: `chrome://extensions` → Developer mode → Load unpacked → папка `extension/`. Настройки расширения: API-ключ, URL AnkiConnect, колода `Cockpit`.
- `docs/` — PRD и план доработок 2.0.
- В Настройках cockpit есть кнопка «Проверить связь» с AnkiConnect.
